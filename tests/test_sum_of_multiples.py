...
def test_basic_multiples():
    ...
    assert sum_of_multiples(10, [3, 5]) == 23  # 3 + 5 + 6 + 9 (NOT 10)
...
def test_no_multiples():
    """Test when no multiples found."""
    assert sum_of_multiples(2, [7]) == 0

def test_multiple_overlap():
    """Test handling of overlapping multiples."""
    assert sum_of_multiples(20, [3, 5]) == 78  # Unique multiples: 3, 5, 6, 9, 10, 12, 15, 18
...
def test_print_multiples():
    ...
    for multiple in range(5, 1000, 5):
        if multiple not in multiples:
            multiples.append(multiple)
    print(f"Multiples: {sorted(multiples)}")
    print(f"Sum of multiples: {sum(multiples)}")
